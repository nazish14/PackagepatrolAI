...
def save_scan(name, manager, score, level, findings):
    ...
    for finding in findings:
        ...
        if severity in {"HIGH", "MEDIUM"} and category != "normal":
            threat = finding.get(
                "title",
                "Security indicator",
            )
...
def badge(level):
    return (
        f'<span class="pp-badge {level_class(level)}">'
        f'{level}'
        f'</span>'
    )
...
def render_sidebar():
    with st.sidebar:
        ...
        st.caption(
            "PackagePatrol AI helps developers detect "
            "vulnerabilities, malicious packages, typosquats "
            "and risky dependencies before they reach production."
        )
        ...
        st.caption(
            "Defensive scanner: metadata, heuristics and "
            "vulnerability lookup. It never installs or "
            "executes packages."
        )
...
def render_hero():
    st.markdown(
        ...
        <div class="pp-hero">
            ...
            <p class="pp-muted">
                Detect vulnerabilities, malicious packages and
                risky dependencies in your open-source ecosystem.
            </p>
...
def render_kpis(history):
    ...
    if not history.empty:
        ...
        last_scan = pd.to_datetime(
            history.iloc[0]["timestamp"]
        ).strftime("%d %b %Y")
...
def run_scan(name, manager):
    ...
    if not valid_package_name(
        ...
        st.error(
            "Invalid package name. Please verify the "
            "name for the selected ecosystem."
        )
        return
    ...
    with st.status(
        "Running PackagePatrol security checks…",
        expanded=True,
    ) as status:
        try:
            ...
            st.write(
                "Checking vulnerabilities and package signals…"
            )
            ...
            if result is None:
                ...
                st.error(
                    "Package not found. Please verify the "
            ...
            st.error(
                "Unable to retrieve package information or "
...
def render_scan_form():
    ...
    st.caption(
        "Enter a package name and optional version to check "
        "available security signals."
    )

    with st.container(border=True):
        column_one, column_two = st.columns([2, 1])

        with column_one:
            st.text_input(
                "Package Name",
                key="package_input",
                placeholder="requests, lodash, express…",
            )
    ...
    for column, item in zip(demo_columns, demos):
        name, manager = item
...
def render_security_checks(analysis):
    ...
    categories = {
        "Known Vulnerabilities": any(
            finding.get("category") == "known_security"
            for finding in findings
        ),
        "Malicious Package": any(
            "malicious" in str(finding).lower()
            for finding in findings
        ),
        "Typosquat Risk": bool(
            analysis.get("typosquatting")
        ),
        "Install Script Risk": any(
            "install" in str(finding).lower()
            for finding in findings
        ),
    }
        ...
        with column:
            ...
            st.markdown(
                ...
                <div class="pp-card">
                    ...
                    <h3>{icon} {state}</h3>
                    <div class="pp-small">
                        Based on available scanner evidence
                    </div>
                </div>
                """,
                unsafe_allow_html=True,
...
def render_findings(analysis):
    ...
    if not findings:
        st.info("No security findings were returned.")
        return
    ...
    for finding in findings:
        ...
        with st.expander(
            f"{title} · {severity}",
            expanded=severity in {"HIGH", "MEDIUM"},
        ):
            st.markdown(
                f"**Severity:** {badge(severity)}",
                unsafe_allow_html=True,
            )
...
def render_ai_explanation(result):
    ...
    st.info(
        "Press the button below to generate a "
        "plain-language security explanation for this scan."
    )
    ...
    if st.button(
        ...
        with st.spinner(
            "AI security analyst is reviewing the scan..."
        ):
            try:
...
def render_result():
    ...
    with right:
        ...
        actions = {
            "LOW": [
                "Keep using this version if it meets your needs.",
                "Monitor new advisories.",
                "Consider dependency lock files.",
            ],
            "MEDIUM": [
                "Review the dependency and its findings.",
                "Check available updates.",
                "Monitor advisories before deployment.",
            ],
            "HIGH": [
                "Upgrade or replace the dependency.",
                "Review dependency usage and package source.",
                "Consider blocking deployment until reviewed.",
                "Investigate suspicious behavior.",
            ],
            "CRITICAL": [
                "Block deployment until investigated.",
                "Replace or remove the dependency.",
                "Review package source and transitive impact.",
            ],
        }
    ...
    if reasons:
        ...
        st.caption(
            "No additional scored findings were returned."
        )
    ...
    details = {
        ...
        "Ecosystem": package.get("manager"),
        "Version": package.get("version"),
        "Author / maintainer": package.get("author"),
        "Description": package.get("description"),
        "License": package.get("license"),
        "First release": package.get("release_date") or "N/A",
        "Last updated": package.get("last_updated") or "N/A",
        "Versions": package.get("versions_count"),
        "Dependencies": len(
...
def render_dashboard():
    ...
    if history.empty:
        st.info(
            "No scans yet. Run your first package scan "
...
def render_history():
    ...
    if manager_filter != "All":
        filtered = filtered[
            filtered.manager == manager_filter
        ]
    ...
    if package_filter:
        filtered = filtered[
            filtered.package_name.str.contains(
                package_filter,
                case=False,
                na=False,
            )
...
def render_settings():
    ...
    st.write(
        "Supported ecosystems: **PyPI** and **npm**"
    )

    st.write(
        "The scanner uses registry metadata, heuristic "
        "signals and OSV vulnerability records."
    )

    st.markdown("### AI configuration")

    st.write(
        "Set `GROQ_API_KEY` in Streamlit Secrets or "
        "as an environment variable to enable AI explanations."
    )

    st.markdown("### Data")

    st.write(
        f"SQLite database: `{DB_FILE}`"
    )

    st.caption(
        "No packages are installed or executed by this application."
    )
