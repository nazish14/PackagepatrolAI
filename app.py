...
def run_scan(name, manager):
    ...
    if not valid_package_name(package_name, manager):
        st.error(
            "Invalid package name. Please verify the name "
            "for the selected ecosystem."
        )
        return
...
def render_result():
    ...
    if st.session_state.ai_explanation:
        ...
        with st.spinner(
            "Generating an evidence-based explanation…"
        ):
            try:
                st.session_state.ai_explanation = (
...
def render_settings():
    ...
    st.write(
        "Set `GROQ_API_KEY` in Streamlit Secrets or as "
        "an environment variable to enable AI explanations."
    )
